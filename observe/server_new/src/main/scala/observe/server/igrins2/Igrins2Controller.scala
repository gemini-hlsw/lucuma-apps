...
enum Igrins2ControllerState(val tag: String) derives Enumerated:
  case Idle        extends Igrins2ControllerState("IDLE")
  case Exposing    extends Igrins2ControllerState("EXPOSING")
  case ReadingOut  extends Igrins2ControllerState("FOWLER_BACK")
  case CreatingMEF extends Igrins2ControllerState("CREATING MEF")
  case Error       extends Igrins2ControllerState("ERROR")
...
object Igrins2Controller:
  def apply[F[_]: Sync: Logger](client: Igrins2Client[F], gds: GdsClient[F]): Igrins2Controller[F] =
    new AbstractGiapiInstrumentController[F, Igrins2Config, Igrins2Client[F]](
      client,
      TimeSpan.unsafeFromMicroseconds(GiapiClient.DefaultCommandTimeout.toMicros)
