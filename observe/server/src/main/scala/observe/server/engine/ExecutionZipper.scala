...
case class ExecutionZipper[F[_]](
  ...
  /**
   * Adds the `Current` `Execution` to the list of completed `Execution`s and makes the next pending
   * `Execution` the `Current` one.
   *
   * If there are still `Action`s that have not finished in `Current` or if there are no more
   * pending `Execution`s it returns `None`.
   */
  lazy val withNextExecution: Option[ExecutionZipper[F]] =
    pending match {
      case Nil           => None
      case exep :: exeps =>
   ...
   */
  lazy val uncurrentify: Option[EngineStep[F]] =
    if (pending.isEmpty)
   ...
   * `Execution`s.
   */
  lazy val toEngineStep: EngineStep[F] =
    EngineStep(
      id = id,
...
object ExecutionZipper {
  ...
  /**
   * Make a `Zipper` from a `Step` only if all the `Execution`s in the `Step` are pending. This is a
   * special way of *zipping* a `Step`.
   */
