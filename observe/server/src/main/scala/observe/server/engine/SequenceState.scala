...
final case class SequenceState[F[_]](
  ...
  def start(i: Int): SequenceState[F] =
    loadedStep match
      case None     => this
      case Some(ls) =>
        copy(loadedStep = Some(ls.start(i)),
             singleRuns = singleRuns.updated(ls.id, ls.resources.map(_ -> ActionState.Idle).toMap)
        )
  ...
  def startSingle(c: ConfigActionCoords): SequenceState[F] =
    copy(singleRuns =
      singleRuns.updatedWith(c.stepId)(
        _.map(_.updatedWith(c.system)(_ => ActionState.Started.some))
          .orElse(Map(c.system -> ActionState.Started).some)
      )
    )
  ...
  def failSingle(c: ConfigActionCoords, err: Result.Error): SequenceState[F] =
    ...
    then
      copy(singleRuns =
        singleRuns.updatedWith(c.stepId)(
          _.map(_.updatedWith(c.system)(_ => ActionState.Failed(err).some))
        )
      )
    else this
  ...
  def completeSingle[V <: RetVal](c: ConfigActionCoords, r: V): SequenceState[F] =
    ...
    then
      copy(singleRuns =
        singleRuns.updatedWith(c.stepId)(
          _.map(_.updatedWith(c.system)(_ => ActionState.Completed(r).some))
        )
      )
    else this
