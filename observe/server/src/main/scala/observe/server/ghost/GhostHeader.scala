...
object GhostHeader {
  ...
  def header[F[_]: MonadThrow as F](
        ...
        GdsInstrument
          .bundleKeywords[F](
            List(
              ...
              buildString(s"$id.fits".pure, KeywordName.ORIGNAME)
            )
          )
          // This is required for GDS 1.x currently only for GHOST/GPI 1
          .flatMap(gdsClient.openObservation(obsId, id, _))
