...
/**
 * Navigate's GraphQL schema. `NavigateDB.graphql` is `navigate.graphql` with its imports from the
 * ODB schema resolved. It is checked against the original by Navigate's `NavigateSchemaSuite` and
 * regenerated with
 * `sbt "navigate_web_server/Test/runMain navigate.web.server.http4s.RenderNavigateSchema"
 * observe_server/clueClean`. Clue only regenerates code when the queries change, hence the
 * `clueClean`.
 */
@GraphQLSchema
trait NavigateDB {
  ...
  object Types {
    type OffsetComponentInput = lucuma.schemas.ObservationDB.Types.OffsetComponentInput
    type OffsetInput          = lucuma.schemas.ObservationDB.Types.OffsetInput
    type WavelengthInput      = lucuma.schemas.ObservationDB.Types.WavelengthInput
  }
}
