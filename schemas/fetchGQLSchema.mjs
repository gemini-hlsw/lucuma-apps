...
const url = endpoints[server]?.[process.argv[2]] ?? usage();
...
console.log(`Fetched ${server} schema from ${url}`);
...
const outputFile = outFiles[server];

await writeFile(outputFile, schema);
console.log(`Wrote ${server} schema to ${outputFile}.`);
