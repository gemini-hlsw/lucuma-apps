...
object Dependencies {
  ...
  private def deps(modules: OrganizationArtifactName*)(version: String): Seq[ModuleID] =
    modules.map(_ % version)

  // Pins the JVM artifact even inside a cross project, where `%%` would otherwise look for
  // a Scala.js build that was never published.
  private def depsJVM(modules: OrganizationArtifactName*)(version: String): Seq[ModuleID] =
    modules.map(_ % version).map(_.platform(Platform.jvm))
