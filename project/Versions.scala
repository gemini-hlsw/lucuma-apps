object Versions {
  val acm                    = "0.1.1"
  val boopickle              = "1.5.0"
  val cats                   = "2.13.0"
  val catsEffect             = "3.7.1"
  val catsParse              = "1.1.0"
  val catsRetry              = "4.0.0"
  val catsTestkitScalaTest   = "2.1.5"
  val catsTime               = "0.6.0"
  val circe                  = "0.14.16"
  val circeRefined           = "0.15.1"
  val coulomb                = "0.9.1"
  val clue                   = "0.59.0"
  val crystal                = "0.51.3"
  val discipline             = "1.7.0"
  val disciplineMUnit        = "2.0.0"
  val epicsCa                = "1.3.2"
  val epicsJca               = "2.4.12"
  val fs2                    = "3.14.0"
  val fs2Data                = "1.11.0"
  val fs2Dom                 = "0.3.0-M1"
  val geminiLocales          = "0.12.3"
  val giapiScala             = "0.5.0"
  val grackle                = "0.30.0"
  val graphQLRoutes          = "0.16.0"
  val http4s                 = "0.23.37"
  val http4sDom              = "0.2.12"
  val http4sJdkHttpClient    = "0.10.0"
  val http4sOtel4s           = "0.19.0"
  val http4sScalaXml         = "0.24.1"
  val javaTimeJs             = "2.7.0"
  val jwt                    = "11.0.4"
  val kittens                = "3.5.0"
  val log4Cats               = "2.8.0"
  val log4CatsLogLevel       = "0.3.1"
  val log4s                  = "1.10.0"
  val logback                = "1.6.4"
  val lucumaCore             = "0.253.0-8-6fc520a-20260924T140338Z-SNAPSHOT"
  val lucumaPrimeStyles      = "0.5.0"
  val lucumaReact            = "0.106.0"
  val lucumaRefined          = "0.1.4"
  val lucumaServers          = "0.97.0-40-0a58f75-20260924T141940Z-SNAPSHOT"
  val monocle                = "3.3.0"
  val mouse                  = "1.4.0"
  val mUnit                  = "1.3.6"
  val mUnitCatsEffect        = "2.2.1"
  val mUnitScalacheck        = "1.3.1"
  val openTelemetry          = "1.64.0" // must match otel4s, see .scala-steward.conf
  val openTelemetryInstr     = "2.26.1-alpha"
  val otel4s                 = "1.1.0"
  val otel4sSdk              = "0.19.4"
  val pprint                 = "0.9.6"
  val pureConfig             = "0.17.10"
  val sbtBuildInfo           = "0.13.2"
  val sbtLucuma              = "0.16.5"
  val sbtRevolver            = "0.10.0"
  val scalaCollectionContrib = "0.4.0"
  val scalaJsDom             = "2.8.1"
  val scalaJsReact           = "4.0.0"
  val slf4j                  = "2.0.20"
  val unboundId              = "3.2.1"
}
