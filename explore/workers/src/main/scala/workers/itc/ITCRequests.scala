...
object ITCRequests:
  val cacheVersion: CacheVersion = CacheVersion(43)
  ...
  def queryItc[F[_]: {Concurrent, Parallel, Logger}](
    ...
    val itcRowsParams: List[ItcRequestParams] =
      modes
        ...
        .collect:
          case m @ ItcInstrumentConfig.GmosNorthSpectroscopy(_, _, _, _, _, _)   =>
            ItcRequestParams(constraints, asterism, customSedTimestamps, m)
          case m @ ItcInstrumentConfig.GmosSouthSpectroscopy(_, _, _, _, _, _)   =>
            ItcRequestParams(constraints, asterism, customSedTimestamps, m)
          ...
          case m @ ItcInstrumentConfig.GhostIfu(_, _, _, _, _)                   =>
            ItcRequestParams(constraints, asterism, customSedTimestamps, m)
          case m @ ItcInstrumentConfig.GnirsSpectroscopy(_, _, _, _, _, _, _, _) =>
            ItcRequestParams(constraints, asterism, customSedTimestamps, m)
          case m @ ItcInstrumentConfig.GnirsImaging(_, _, _, _, _)               =>
            ItcRequestParams(constraints, asterism, customSedTimestamps, m)
