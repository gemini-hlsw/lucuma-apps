...
trait WorkerServer[T: Pickler](using Monoid[IO[Unit]]):
  protected val run: IO[Unit] =
    (for {
      ...
      endpointDeferred        <- Resource.eval(Deferred[IO, Option[String]])
      // Phase 1: capture tracing endpoint from ClientReady before building SDK
      _                       <- Resource.eval(IO.delay {
                                   self.onmessage = (msg: dom.MessageEvent) =>
                                     dispatcher.unsafeRunAndForget(
                                       decodeFromTransferable[FromClient](msg).map {
                                         case FromClient.ClientReady(ep) =>
                                           endpointDeferred.complete(ep).void
                                         case _                          => IO.unit
                                       }.orEmpty
                                     )
                                 })
      endpoint                <- Resource.eval(endpointDeferred.get)
      otel                    <- WorkerOtelSdk.build(endpoint)
      _                       <- Resource.eval(runInternal(dispatcher, self, otel))
   ...
   */
  protected def handler: (LoggerFactory[IO], Tracer[IO]) ?=> IO[Invocation => IO[Unit]]
