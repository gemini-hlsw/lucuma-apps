...
import lucuma.schemas.ObservationDB.Types.GmosNorthIfuInput
...
import lucuma.schemas.ObservationDB.Types.GmosSouthIfuInput
...
enum ObservingModeSummary derives Order:
  ...
  def obsModeType: ObservingModeType = this match
    ...
    case GmosNorthIfu(_, _, _, _, _)                            => ObservingModeType.GmosNorthIfu
    case GmosSouthIfu(_, _, _, _, _)                            => ObservingModeType.GmosSouthIfu
  ...
  def toInput: ObservingModeInput = this match
    ...
    case GmosNorthMos(grating, filter, slitWidth, centralWavelength, etm)                  =>
    ...
    case GmosSouthMos(grating, filter, slitWidth, centralWavelength, etm)                  =>
...
object ObservingModeSummary:
  ...
  def fromObservingMode(observingMode: ObservingMode): ObservingModeSummary =
    observingMode match
      ...
      case m: ObservingMode.GmosNorthIfu       =>
        GmosNorthIfu(
          m.grating,
          m.filter,
          m.fpu,
      ...
      case m: ObservingMode.GmosSouthIfu       =>
        GmosSouthIfu(
          m.grating,
          m.filter,
          m.fpu,
  ...
  given Display[ObservingModeSummary] = Display.byShortName:
    ...
    case GmosNorthMos(grating, filter, slitWidth, centralWavelength, _)                  =>
      val cwvStr    = "%.1fnm".format(centralWavelength.value.toNanometers)
      val filterStr = filter.fold("None")(_.shortName)
      s"GMOS-N MOS ${grating.shortName} @ $cwvStr $filterStr  ${slitWidth.shortName}"
    case GmosSouthMos(grating, filter, slitWidth, centralWavelength, _)                  =>
      val cwvStr    = "%.1fnm".format(centralWavelength.value.toNanometers)
      val filterStr = filter.fold("None")(_.shortName)
