...
final case class AttachmentsTile(
...
object AttachmentsTileHelpers {
  def includedPurposes(showObsAttachments: Boolean) =
    if (showObsAttachments)
      Set(AttachmentPurpose.Observation, AttachmentPurpose.Target)
    else Set(AttachmentPurpose.Target)

  def count(attachments: View[AttachmentList], showObsAttachments: Boolean) =
...
object AttachmentsTile
    extends TileComponent[AttachmentsTile]({ (props, _) =>
      ...
      def columns(ctx: AppContext[IO], props: AttachmentsTile, action: View[Action]) = {
        import ctx.given
        ...
        def goToTarget(targetId: Target.Id): Callback =
          ctx.pushPage((AppTab.Targets, props.programId, Focused.target(targetId)).some)

        def targetUrl(targetId: Target.Id): String =
          ctx.pageUrl((AppTab.Targets, props.programId, Focused.target(targetId)).some)
      ...
      for {
        ...
        client        <- useMemo(props.userVault.token): token =>
        ...
        columns       <- useMemo(())(_ => columns(ctx, props, action))
        rows          <- useMemo((props.showObsAttachments, props.attachments.reuseByValue)):
                           (showObsAttachments, attachments) =>
                             val included = AttachmentsTileHelpers.includedPurposes(showObsAttachments)
