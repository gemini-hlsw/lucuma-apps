...
/** Shared field block for the visitor configuration UI.
  *
  * Used in two contexts:
  *   - ResidentVisitorConfigPanel (editing an existing observation; views derived from an Aligner).
  *   - AlienVisitorConfigEditor (pre-creation; views from local state).
  */
final case class VisitorConfigFields(
  header:               VdomNode,
...
object VisitorConfigFields
    extends ReactFnComponent[VisitorConfigFields](props =>
      val cwId  = NonEmptyString.unsafeFrom(s"${props.idPrefix.value}-central-wavelength")
      val fovId = NonEmptyString.unsafeFrom(s"${props.idPrefix.value}-science-fov")
