...
class TestChannel[F[_]: Temporal, S, A: Eq](s: Ref[F, S], l: Lens[S, State[A]])
    extends Channel[F, A] {
  override val get: F[A] = s.get.flatMap(x => l.get(x).value.map(_.pure[F]).getOrElse(Temporal[F].raiseError(new Throwable("TestChannel.get: Cannot read channel, value is empty."))))
