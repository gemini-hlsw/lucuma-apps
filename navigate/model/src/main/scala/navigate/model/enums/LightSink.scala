...
enum LightSink(val tag: String, val instrument: Instrument, val variant: Option[LightSinkVariant])
  ...
  case GmosNorthIfu
      extends LightSink("GmosNorthIfu", Instrument.GmosNorth, LightSinkVariant.GmosIfu.some)
  case GmosSouthIfu
      extends LightSink("GmosSouthIfu", Instrument.GmosSouth, LightSinkVariant.GmosIfu.some)
...
object LightSink {
  def fromInstrumentAndVariant(
    instrument: Instrument,
    variant:    Option[LightSinkVariant]
  ): Option[LightSink] =
    LightSink.values.find(x => x.instrument === instrument && x.variant === variant)
}
