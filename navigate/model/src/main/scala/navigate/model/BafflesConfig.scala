...
object BafflesConfig {
  ...
  extension (ac: AutoConfig) {
    def toManualConfig(centralWavelength: Wavelength, instrument: Instrument): ManualConfig = instrument match {
      case Instrument.Gpi => ManualConfig(CentralBafflePosition.Closed, DeployableBafflePosition.ThermalIr)
      case _ => if (
        centralWavelength <= ac.visibleLimit
      )
  ...
  extension (bc: BafflesConfig) {
    def deployable(centralWavelength: Wavelength, instrument: Instrument): DeployableBafflePosition = bc match {
      case ManualConfig(_, deployable) => deployable
      case x: AutoConfig               => x.toManualConfig(centralWavelength, instrument).deployable
    }
    def central(centralWavelength: Wavelength, instrument: Instrument): CentralBafflePosition       = bc match {
