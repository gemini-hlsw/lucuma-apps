...
object BafflesConfig {
  ...
  extension (ac: AutoConfig) {
    def toManualConfig(centralWavelength: Wavelength): ManualConfig = if (
  ...
  extension (bc: BafflesConfig) {
    def deployable(centralWavelength: Wavelength): DeployableBafflePosition = bc match {
      case ManualConfig(_, deployable) => deployable
      case x: AutoConfig               => x.toManualConfig(centralWavelength).deployable
    }
    def central(centralWavelength: Wavelength): CentralBafflePosition       = bc match {
      case ManualConfig(central, _) => central
      case x: AutoConfig            => x.toManualConfig(centralWavelength).central
    }
  }
